...
// The current spec surface, plus two members the draft removed that Chrome's
// early builds still ship (`navigator.modelContext`, `unregisterTool`); only
// `getModelContext()` and `useWebmcpTools` touch them, as a fallback.
 ...
 * tools whose results can carry attacker-written text (customer messages)
 * so the agent treats it as data.
 */
...
interface ModelContext extends EventTarget {
  ...
  /** Removed from the draft; early Chrome builds unregister only through it. */
...
/**
 * Present only in WebMCP-capable browsers (Chrome origin trial 149 to 156).
...
/** Pre-draft location, still the only one in Chrome 148. */
interface Navigator {
  readonly modelContext?: ModelContext
}
