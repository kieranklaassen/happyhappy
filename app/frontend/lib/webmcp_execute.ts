...
/** Every manifest kind happyhappy ships runs through the interpreter. */
export type WebmcpInterpretedTool = WebmcpManifestTool

interface ExecuteOptions {
  /** The registration run's signal; cancels the fetch when the page goes away. */
  signal?: AbortSignal
  /** Merged into results of tools flagged `include_viewer_context`. */
...
const TOOL_PATH_PREFIX = '/webmcp/tools/'
...
/** The cap-exceeded error shape, so an agent sees one vocabulary for "too long". */
...
/**
 * Turns a manifest tool plus arguments into a same-origin fetch and an
 * MCP-style result. Never throws and never returns `undefined`: one outer
 * boundary converts every failure (identity, allowlist, params, headers,
 * abort, network, parse) into an `isError` envelope.
 */
...
async function requestResult(
  ...
  // Identity gate first: no network request is made without a name.
  ...
  const init: RequestInit = {
    ...
    headers,
    // The signed-in session is the credential: WebmcpToolsController attributes
    // writes to the person's browser agent and checks the CSRF token above.
  ...
  if (response.status === 429 && typeof request.rate_limit_window_seconds === 'number') {
    ...
    // lockout would over-block for the whole window after one 429.
...
/**
 * Keeps every request on the page origin under `/webmcp/tools/` before any
 * path substitution, then fills `:name` placeholders with digit-only,
 * URL-encoded values so an agent-supplied `id` can never retarget the request
 * to another same-origin route.
 */
function resolveUrl(
  template: string,
  pathParams: string[],
  ...
  if (!path.startsWith(TOOL_PATH_PREFIX)) {
    return { error: `refused: ${path} is outside ${TOOL_PATH_PREFIX}` }
  }
...
function csrfTokenFromPage(): string | null {
